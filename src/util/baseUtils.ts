 ...
 */
export function checkSupportedResource(resourceType: string) {
 ...
 */
export function checkExpectedResource(resourceType: string, expectedResourceType: string) {
 ...
 */
export function checkContentTypeHeader(requestHeaders: Record<string, string>) {
  if (
...
export function createPaginationLinks(
  ...
  if (searchParams.toString() !== '') {
    links.push({
      relation: 'self',
      url: new url.URL(`${resourceType}?${searchParams}`, baseUrl).toString()
    });
  } else {
    links.push({
      relation: 'self',
      url: new url.URL(`${resourceType}`, baseUrl).toString()
    });
  }
